...
public class Level2Map implements LevelMap {
    ...
    // A large 17x9 market square with 4 booths in the corners!
    public static final String[] LEVEL_2_LAYOUT = {
        ...
        "W.BCB.......BCB.W",
        "W...............W",
        "WWWWWWWWWDWWWWWWW"  // The exit door (D) right in the middle
    };
    ...
    public void load(EntityManager entityManager, TextureCache textures) {
        ...
        for (int row = 0; row < LEVEL_2_LAYOUT.length; row++) {
            ...
            for (int col = 0; col < line.length(); col++) {
                ...
                // 2. Generate specific wall/counter types
                if (tile == 'W') {
                    float rotation = 0f;
                    float zOffset = 0f;
                    float xOffset = 0f;
        ...
        for (int row = 0; row < LEVEL_2_LAYOUT.length; row++) {
            ...
            for (int col = 0; col < line.length(); col++) {
                if (line.charAt(col) == 'D') {
                    return new Vector3(col * TILE_SIZE, 1.0f, (row * TILE_SIZE) - 1.0f);
                }
            }
        }
