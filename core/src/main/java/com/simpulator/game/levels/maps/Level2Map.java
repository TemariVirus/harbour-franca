...
public class Level2Map implements LevelMap {
    ...
    private boolean isBooth(String[] layout, int r, int c) {
        if (r < 0 || r >= layout.length || c < 0 || c >= layout[0].length()) return false;
    ...
    public void load(EntityManager entityManager, TextureCache textures) {
        ...
        for (int row = 0; row < LEVEL_2_LAYOUT.length; row++) {
            ...
            for (int col = 0; col < line.length(); col++) {
                ...
                if (tile == 'P') {
                    entityManager.add(new CuboidEntity(new Vector3(x, -1.0f, z), new Vector3(TILE_SIZE, TILE_SIZE, 0.1f), new Quaternion().setFromAxis(Vector3.X, -90), pavementTex, false));
                } else if (tile == '.' || tile == 'C' || tile == 'M' || tile == 'B') {
                    entityManager.add(new CuboidEntity(new Vector3(x, -1.0f, z), new Vector3(TILE_SIZE, TILE_SIZE, 0.1f), new Quaternion().setFromAxis(Vector3.X, -90), floorTex, false));
                }
                
                if (tile == 'W') {
                    float rotation = 0f, zOffset = 0f, xOffset = 0f;
                    if (row == 0) { rotation = 0f; zOffset = 1.0f; } 
                    else if (row == LEVEL_2_LAYOUT.length - 1) { rotation = 180f; zOffset = -1.0f; } 
                    else if (col == 0) { rotation = 90f; xOffset = 1.0f; } 
                    else if (col == line.length() - 1) { rotation = -90f; xOffset = -1.0f; }

                    entityManager.add(new CuboidEntity(
                        new Vector3(x + xOffset, 1.0f, z + zOffset), 
                        new Vector3(TILE_SIZE, 4.0f, TILE_SIZE), 
                        new Quaternion().setFromAxis(Vector3.Y, rotation), outerWallTex, false
                    ));
                } 
                // Edge Snapping
                else if (tile == 'B') {
                    ...
                    if (!n) { // North Wall 
                        entityManager.add(new CuboidEntity(new Vector3(x, 1.0f, z - offset), new Vector3(TILE_SIZE, 4.0f, TILE_SIZE), new Quaternion().setFromAxis(Vector3.Y, 180f), boothWallTex, false)); // Outward Face
                        entityManager.add(new CuboidEntity(new Vector3(x, 1.0f, z - offset), new Vector3(TILE_SIZE, 4.0f, TILE_SIZE), new Quaternion().setFromAxis(Vector3.Y, 0f), boothWallTex, false));   // Inward Face
                    }
                    if (!s) { // South Wall
                        entityManager.add(new CuboidEntity(new Vector3(x, 1.0f, z + offset), new Vector3(TILE_SIZE, 4.0f, TILE_SIZE), new Quaternion().setFromAxis(Vector3.Y, 0f), boothWallTex, false));
                        entityManager.add(new CuboidEntity(new Vector3(x, 1.0f, z + offset), new Vector3(TILE_SIZE, 4.0f, TILE_SIZE), new Quaternion().setFromAxis(Vector3.Y, 180f), boothWallTex, false));
                    }
                    if (!w) { // West Wall
                        entityManager.add(new CuboidEntity(new Vector3(x - offset, 1.0f, z), new Vector3(TILE_SIZE, 4.0f, TILE_SIZE), new Quaternion().setFromAxis(Vector3.Y, -90f), boothWallTex, false));
                        entityManager.add(new CuboidEntity(new Vector3(x - offset, 1.0f, z), new Vector3(TILE_SIZE, 4.0f, TILE_SIZE), new Quaternion().setFromAxis(Vector3.Y, 90f), boothWallTex, false));
                    }
                    if (!e) { // East Wall
                        entityManager.add(new CuboidEntity(new Vector3(x + offset, 1.0f, z), new Vector3(TILE_SIZE, 4.0f, TILE_SIZE), new Quaternion().setFromAxis(Vector3.Y, 90f), boothWallTex, false));
                    ...
                    entityManager.add(new CuboidEntity(
                        ...
                        new Quaternion().setFromAxis(Vector3.Y, counterRot), counterTex, false
                    ));
                }
            }
        }
        ...
        for (int row = 0; row < LEVEL_2_LAYOUT.length; row++) {
            ...
            for (int col = 0; col < line.length(); col++) {
                if (line.charAt(col) == 'D') {
                    return new Vector3((col * TILE_SIZE) + SMALL_OFFSET, 1.0f, (row * TILE_SIZE) - 1.0f + SMALL_OFFSET);
                }
            }
        }
